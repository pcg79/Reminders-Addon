...
local function UpdateFrame(frame, i)
  ...
  frame.pageNum:SetText(("%d/%d"):format(i, frame.maxFrames))
  if i < (frame.maxFrames or 0) then
    frame.next:Enable()
  else
    frame.next:Disable()
  end
...
local function NewFrame(data)
  ...
  if data.font then
    frame.text:SetFont(data.font, data.fontHeight)
  end
...
function Reminders:DisplayReminders(data)
  ...
  local frame = ReminderFrame

  if frame then
    local count = 0
    for _ in pairs(frame.data.reminders) do count = count + 1 end
    frame.maxFrames = count
